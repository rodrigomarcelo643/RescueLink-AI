  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="relative w-full max-w-2xl bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden flex flex-col max-h-[90vh]">
        ...
        {/* Modal Header */}
        <div className="flex items-start justify-between p-5 border-b border-gray-100 bg-gray-50/80">
          <div className="flex flex-col gap-1.5 pr-6">
            <div className="flex items-center gap-2 flex-wrap">
              <span className={`px-2.5 py-0.5 text-[10px] font-black uppercase rounded ${sevCfg.bg} ${sevCfg.color}`}>
                {sevCfg.label}
              </span>
              <span className={`px-2.5 py-0.5 text-[11px] font-bold rounded ${statCfg.bg} ${statCfg.color}`}>
                {statCfg.label}
              </span>
              {distanceText && (
                <span className="px-2.5 py-0.5 text-[10px] font-extrabold bg-blue-100 text-blue-800 rounded">
        ...
        <div className="p-5 overflow-y-auto flex flex-col gap-4 text-xs font-sans">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            <div className="p-3 bg-gray-50 rounded-xl border border-gray-150 flex flex-col gap-1">
              <span className="text-[10px] font-extrabold uppercase text-gray-400">People Affected</span>
            ...
            <div className="p-3 bg-gray-50 rounded-xl border border-gray-150 flex flex-col gap-1">
              <span className="text-[10px] font-extrabold uppercase text-gray-400">Assigned Response</span>
            ...
            <div className="p-3 bg-gray-50 rounded-xl border border-gray-150 flex flex-col gap-1 col-span-2 sm:col-span-1">
              <span className="text-[10px] font-extrabold uppercase text-gray-400">Reported Time</span>
          ...
          {lat && lng && (
            <div className="p-3 bg-gray-50 rounded-xl border border-gray-150 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <span className="text-[10px] font-extrabold uppercase text-gray-400 block">GPS Coordinates</span>
          ...
          {mediaUrls.length > 0 && (
            <div className="flex flex-col gap-2 pt-2 border-t border-gray-100">
              <span className="text-xs font-extrabold text-gray-900 flex items-center gap-1.5">
                ...
                Photo & Media Proof ({mediaUrls.length})
              </span>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {mediaUrls.map((url, idx) => (
                  <img
                    ...
                    className="h-28 w-full object-cover rounded-lg border border-gray-200 hover:opacity-90 transition-opacity cursor-pointer shadow-2xs"
                  />
                ))}
              </div>
            </div>
        ...
        <div className="p-4 border-t border-gray-100 bg-gray-50 flex items-center justify-between gap-3">
          <button
            onClick={handleShare}
            className="flex items-center gap-1.5 px-3.5 py-2 text-xs font-bold bg-white text-gray-700 hover:bg-gray-100 border border-gray-200 rounded-xl transition-colors cursor-pointer shadow-2xs"
          >
            {copied ? <CheckCircle2 size={14} className="text-green-600" /> : <Share2 size={14} />}
            {copied ? 'Copied Alert!' : 'Share Details'}
          </button>
          ...
          <div className="flex items-center gap-2">
            {incidentObj && (
              <Link
                ...
                onClick={onClose}
                className="flex items-center gap-1.5 px-4 py-2 text-xs font-extrabold text-white bg-red-700 hover:bg-red-800 rounded-xl transition-all shadow-xs"
              >
                Track Live Operation 📍 →
              </Link>
            )}
            <button
              onClick={onClose}
              className="px-3.5 py-2 text-xs font-bold text-gray-600 bg-gray-200 hover:bg-gray-300 rounded-xl transition-colors cursor-pointer"
            >
              Close
            </button>
          </div>
